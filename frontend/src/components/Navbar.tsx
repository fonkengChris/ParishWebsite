...
export default function Navbar() {
  ...
  const [showParishLifeMenu, setShowParishLifeMenu] = useState(false);
  ...
  const parishLifeLinks = [
    { path: "/mass-schedule", label: "Mass Schedule" },
    { path: "/announcements", label: "Announcements" },
    { path: "/events", label: "Events" },
    { path: "/parish-life", label: "Apostolates" },
    { path: "/parish-services", label: "Sacraments" },
  ];
  ...
  const isParishLifeActive = parishLifeLinks.some(
    (link) => location.pathname === link.path,
  );

  const isCatholicFaithActive = catholicFaithLinks.some(
    (link) =>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-[68px] gap-4">
          ...
          <div className="hidden lg:flex items-center gap-1 flex-shrink">
            ...
            {/* Parish Life Dropdown */}
            <div className="relative">
              <button
                onClick={() => setShowParishLifeMenu(!showParishLifeMenu)}
                className={`${pill} flex items-center gap-1 ${isParishLifeActive ? pillActive : pillIdle}`}
              >
                Parish Life
                <svg
                  className={`w-4 h-4 transition-transform duration-200 ${showParishLifeMenu ? "rotate-180" : ""}`}
                  fill="none"
                  strokeLinecap="round"
            ...
            <Link
              to="/contact"
              className={`${pill} ${isActive("/contact") ? pillActive : pillIdle}`}
            >
              Contact
            </Link>
                  ...
                  <>
                    ...
                    <div className="absolute right-0 mt-2 w-56 bg-white rounded-2xl shadow-2xl py-2 z-20 border border-line">
                      <div className="px-4 py-3 border-b border-line bg-primary-50">
                        ...
                        <p className="text-xs text-primary-700 capitalize font-medium">
                          {user.role}
                        </p>
                      </div>
                      <Link
                        to="/profile"
                        onClick={() => setShowUserMenu(false)}
                        className="block px-4 py-2.5 text-sm text-ink-soft hover:bg-ivory-2 hover:text-ink transition-colors duration-200"
                      >
                        View Profile
                      </Link>
                      <Link
                      ...
                      <button
                        ...
                        className="block w-full text-left px-4 py-2.5 text-sm text-red-700 hover:bg-red-50 transition-colors duration-200 font-medium"
                      >
                        Logout
                      </button>
                    </div>
          ...
          <div className="lg:hidden py-4 space-y-1.5 bg-white rounded-2xl mt-1 mb-3 border border-line shadow-lg px-2">
            ...
            <div>
              ...
              >
                Parish Life
                <svg
                  className={`w-5 h-5 ml-2 transition-transform duration-200 ${showParishLifeMenu ? "rotate-180" : ""}`}
                  fill="none"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth="2"
              ...
              <div className="px-2 py-2 space-y-1 bg-ivory-2 rounded-xl">
                <div className="flex items-center space-x-3 px-2 py-2 border-b border-line">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-ink-soft capitalize font-medium">
                      {user.role}
                    </p>
                  </div>
                </div>
                <Link
                  to="/profile"
                  onClick={() => setIsOpen(false)}
                  className="block px-4 py-2.5 rounded-xl text-base font-medium text-ink-soft hover:bg-white hover:text-ink transition-all duration-200"
                >
                  View Profile
                </Link>
                <Link
